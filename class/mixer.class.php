...
class Mixer 
{
	...
	{ 	
		...
		$sid = session_id();
		$mayorista = (isset($_SESSION['mayoristas'])) ? $_SESSION['mayoristas'] : 0;

		$query = "SELECT id_mix FROM tbl_cart_mixer WHERE session_id = '$sid' AND tipo_mixer=$this->type AND mayorista=$mayorista ";
		...
		if ($mixer == 0) {
			...
			$query = "INSERT INTO tbl_cart_mixer (peso, tipo_mixer, pack, session_id, mayorista, date)
			VALUES ('$this->peso', '$this->type', '$this->pack', '$sid', '$mayorista', NOW())";
			...
			WHERE session_id = '$sid' AND tipo_mixer=$this->type AND mayorista=$mayorista";		
	...
	{ 
		...
		$query = "SELECT id_mix, base FROM tbl_cart_mixer WHERE session_id = '$sid' AND tipo_mixer=$this->type AND mayorista=$mayorista ";
		if(!empty($this->mixId)){	
			$query .= " AND tbl_cart_mixer.id_mix = '$this->mixId' ";
		} else {
			$query .= " AND id_prod_creado=0 ";
	...
	{ 
		...
		//verificamos que esté creado el mixer y que haya una base cargada
		$query = "SELECT id_mix, peso, tipo_mixer, base FROM tbl_cart_mixer WHERE session_id = '$sid' AND tipo_mixer=$this->type AND mayorista=$mayorista ";
		if(!empty($this->mixId)){	
			$query .= " AND tbl_cart_mixer.id_mix = '$this->mixId' ";
		} else {
			$query .= " AND id_prod_creado=0 ";
	...
	public function controlPeso($idMix,$tipoMix,$prod,$pesoTotal)
	{
		...
		$qry = (isset($_SESSION['mayoristas'])) ? "SUM(pd_peso_mayo*cantidad) as total_consumido" : "SUM(pd_peso*cantidad) as total_consumido";
		$query = "SELECT ".$qry." FROM tbl_cart_item_mixer 
		...
		$qry = (isset($_SESSION['mayoristas'])) ? "pd_peso_mayo" : "pd_peso";
		$query = "SELECT ".$qry." FROM tbl_variaciones_mixer WHERE vm_producto = '$this->productId' AND vm_tipo_mixer='$this->type'";
		$result=mysqli_query($this->conn,$query);
		$row=$result->fetch_assoc();
		$peso_nuevo=intval((isset($_SESSION['mayoristas'])) ? $row['pd_peso_mayo'] : $row['pd_peso']);
		...
		if($cnt_res>0) {
					...
					if($cntIng>0) {
						while ($row=$result->fetch_assoc()) {
								extract($row);
									...
									if ($es_base=='si') {
										$haybase=true;
										$ingredientes .= ' base';
										$precio_base = (isset($_SESSION['mayoristas'])) ? $pd_precio_mayo : $pd_precio;
									} else {
										$peso_total += $cantidad * ((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso);
										$precio_total += $cantidad * ((isset($_SESSION['mayoristas'])) ? $pd_precio_mayo : $pd_precio);
									}
									$ingredientes .= '">
						...
						if (isset($precio_base)) {
							...
							$precio_base_reduc = ($peso_base * $precio_base) / ((isset($_SESSION['mayoristas'])) ? 1000 : 350);
				...
				if ($rowMix['nombre']=='') {
					echo '<h2 class="title-tubo">Tu Mix '.$tm['tm_titulo'];
					echo (isset($_SESSION['mayoristas'])) ? "<br><small class='text-primary'>Mayorista</small></h2>" : "</h2>";
				} else {
						...
						<div class="footer-tubo px-2">';
							if ($haybase || (!empty($producto_id) && $tipo_mixer!=100) ) {
								...
								if (!$haybase) {
									...
									echo '<p class="price-tubo mt-3">'.$completado.'g por $'.number_format($precio_total,2,',','.');
									echo (isset($_SESSION['mayoristas'])) ? "<small>(5Kg por $".number_format($precio_total*5,2,',','.').")</small></p>" : "</p>";
								  ...
								  if ($completado==$peso) {
									...
									echo '<p class="price-tubo mt-3">'.$peso.'g por $'.number_format($precio_total,2,',','.');
									echo (isset($_SESSION['mayoristas'])) ? "<small>(5Kg por $".number_format($precio_total*5,2,',','.').")</small></p>" : "</p>";
									echo '<a href="'.WEB_ROOT.'mixer-checkout.php?order='.$id_mix.'" class="btn btn-primary w-100 rounded-pill">Terminar</a>';
								}
	...
	public function getMixerContentCheck()
	{
		...
		if($cnt_res>0) {
				...
				echo '<div class="ingredientes-mix py-3 container-fluid">
						...
						$precio_total = (isset($_SESSION['mayoristas'])) ? ($precio_total*5) : $precio_total;
						$peso = (isset($_SESSION['mayoristas'])) ? ($peso*5) : $peso;
	...
	public function getValNutri()
	{
		...
		if($cnt_res>0) {
			...
			while ($row=$result->fetch_assoc()) {
				...
				if ($producto_id) {
					...
					if ($es_base=='si') {
						...
						$peso_ingredientes += $cantidad * ((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso);

						$tot_kcal += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$kcal)/100;
						$tot_hidratos_carbono += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$hidratos_carbono)/100;
						$tot_proteinas += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$proteinas)/100;
						$tot_grasas_totales += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_totales)/100;
						$tot_grasas_saturadas += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_saturadas)/100;
						$tot_grasas_trans += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_trans)/100;
						$tot_grasas_monoinsaturadas += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_monoinsaturadas)/100;
						$tot_grasas_poliinsaturadas += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_poliinsaturadas)/100;
						$tot_colesterol += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$colesterol)/100;
						$tot_fibra_alimentaria += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$fibra_alimentaria)/100;
						$tot_sodio += (((isset($_SESSION['mayoristas'])) ? $pd_peso_mayo : $pd_peso)*$cantidad*$sodio)/100;
					}
			...
			$valoresNutricionales = array(
				...
				"contenido-neto" => (isset($_SESSION['mayoristas'])) ? $peso*5 : $peso
			);
		...
		while($cat=$result->fetch_assoc())
			{
				...
				echo '<div id="'.$cat['ct_alias'].'" style="min-height: 700px; margin-top:1px;">
						<div class="mixer-catalog">
							...
							INNER JOIN tbl_variaciones_mixer ON tbl_variaciones_mixer.vm_producto=tbl_productos_mixer.pd_id
							WHERE pd_categoria LIKE '%$categoria%' AND vm_tipo_mixer='$tipo_mixer' AND status='publicado' ");
									...
									echo '<div class="mixer-ingredient" style="border-left:12px solid '.$cat['ct_color'].'">
											<div class="d-flex flex-column flex-md-row ingredient-teaser">
												...
												<div>
													...
													//precio y boton agregar al mix
													if (isset($_SESSION['mayoristas'])) {
														$precioProd = $prod['pd_precio_mayo'];
														$pesoProd = $prod['pd_peso_mayo'];
													} else {
														$precioProd = $prod['pd_precio'];
														$pesoProd = $prod['pd_peso'];
													}

													echo '<div class="extra-information">$'.$precioProd;
													if ($cat['ct_alias']!=='bases') {
														if ($pesoProd!=0) {
															echo '<span class="item-weight">︱'.$pesoProd.'g</span>';
														} else {
	...
	public function etiquetaMixPDF($mix,$token)
	{
		...
		if($cnt_res>0) {
			...
			while ($row=$result->fetch_assoc()) {
				...
				if ($producto_id) {
					...
					if ($es_base=='si') {
						...
						$peso_ingredientes += $cantidad * (($mayorista!=0) ? $pd_peso_mayo : $pd_peso);

						$tot_kcal += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$kcal)/100;
						$tot_hidratos_carbono += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$hidratos_carbono)/100;
						$tot_proteinas += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$proteinas)/100;
						$tot_grasas_totales += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_totales)/100;
						$tot_grasas_saturadas += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_saturadas)/100;
						$tot_grasas_trans += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_trans)/100;
						$tot_grasas_monoinsaturadas += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_monoinsaturadas)/100;
						$tot_grasas_poliinsaturadas += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$grasas_poliinsaturadas)/100;
						$tot_colesterol += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$colesterol)/100;
						$tot_fibra_alimentaria += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$fibra_alimentaria)/100;
						$tot_sodio += ((($mayorista!=0) ? $pd_peso_mayo : $pd_peso)*$cantidad*$sodio)/100;
					}
			...
			$valoresNutricionales = array(
				...
				"ingredientes" => $ingredientes,
				"contenido-neto" => ($mayorista!=0) ? $peso*5 : $peso
			);
