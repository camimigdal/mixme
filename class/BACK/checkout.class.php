...
class Checkout
{
	...
	{		
			...
			// 	$arrDescTransf=$this->descuentoTrasnferencia();
			// 	if ($arrDescTransf) {
			// 		$porcentajeDescuentoTransf=$arrDescTransf[0]['porcentaje_descuento'];
			// 		$descuentoTransf = ($porcentajeDescuentoTransf*$totalSinEnvio)/100;
			// 		$orderAmount=$orderAmount-$descuentoTransf;
			// 	} 
			// }
			...
			$query ="INSERT INTO `ordenes`(`session_id`, `fecha_alta`, `or_nombre`, `or_apellido`, `or_dni`, `or_telefono`, `or_email`, `or_calle`, `or_calle_num`, `or_piso`, `or_depto`, `or_ciudad`, `or_provincia`, `or_codpostal`, `or_medio_pago`, `total_compra`, `or_estado`, `or_notas`) 
			VALUES ('$sid',NOW(),'$per_nombre','$per_apellido','$per_dni','$per_telefono','$per_email','$per_direccion','$per_calle_num','$per_piso','$per_dpto','$per_ciudad','$per_provincia','$per_codpostal','$opcion_pago','$orderAmount',1,'$mensaje')";
			...
			if (mysqli_query($this->conn,$query)) {
				...
				if ($orderId) {
					...
					// 		$desc_descripcion='Descuento de '.$porcentajeDescuentoTransf.'% por transferencia bancaria';
					// 		$query = "INSERT INTO descuentos_orden (id_orden, desc_codigo, desc_descripcion, desc_precio) VALUES ($orderId,'$desc_codigo','$desc_descripcion','$descuentoTransf')";
					// 			if (!mysqli_query($this->conn,$query)) {	
					// 				return false;
					// 			}
					// 	} 
	...
	public function enviaEmailStatus($id_orden,$status) {
		...
		$parametros_post = '{
			...
			"TemplateModel": {
				...
				"text": "'.$text_email.'",
				"company_name": "Mixme",
                "company_address": "Av. Vélez Sarsfield 234, Barracas, Buenos Aires, Argentina."
			}
        }';
