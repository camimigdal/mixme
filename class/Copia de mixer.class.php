...
class Mixer 
{
	...
	{ 		
		...
		if ($mixer == 0) {
			...
			$query = "INSERT INTO tbl_cart_mixer (peso, pack, session_id, date)
			VALUES ('$this->peso', '$this->pack', '$sid', NOW())";
			...
			SET peso = '$this->peso', pack = '$this->pack'
	...
	public function getCartContentAjax()
	{
		...
		if($cnt_res>0) {
				...
				echo '<h2 class="title-tubo">Tu Mix Favorito</h2>
						<div class="ingredientes-tubo py-3 container-fluid">
							<div class="row">
								'.$ingredientes.'
							</div>
						</div>
						<div class="footer-tubo px-2">';
							if ($haybase) {
								...
								<p class="price-tubo mt-3">'.$peso.'g por $'.number_format($precio_total,2,',','.').'</p>
								<a href="mixer-checkout.php?order='.$id_mix.'" class="btn btn-primary w-100 rounded-pill">Terminar</a>
								<a href="#" data-id="'.$id_mix.'" class="btn btn-outline-dark btn-sm rounded-pill mt-3" id="vaciarmix"><i class="far fa-trash-alt"></i> Vaciar el Mix</a>';
							} else {
	...
	public function getMixerContentCheck()
	{
		...
		if($cnt_res>0) {
				...
				echo '<h2 class="title-tubo">Tu Mix Favorito</h2>
						...
						echo '<a href="#val-nut" class="btn btn-outline-dark w-100 rounded-0 my-1">Valores nutricionales</a>
						<p class="price-tubo mt-3">'.$peso.'g por $'.number_format($precio_total,2,',','.').'</p>
						<a href="#" onclick="addMixToCart('.$id_mix.');" class="btn btn-primary w-100 rounded-pill">Agregar al carro</a>';
		...
		while($cat=$result->fetch_assoc())
			{
				...
				echo '<div id="'.$cat['ct_alias'].'" style="min-height: 700px;">
							...
							while($prod=$resultProd->fetch_assoc())
								{
									...
									echo '" data-id="'.$prod['pd_id'].'" style="border-left:12px solid '.$cat['ct_color'].'">
											<div class="d-flex ingredient-teaser">
												...
												<div>
													...
													echo '</div>
													<ul class="list-inline propiedades">';
