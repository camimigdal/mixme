	...
	{ 
		...
		if($cnt_res>0) {
			...
			while ($mix=$result->fetch_assoc()) {
				...
				if ($mix['es_base']=='si') {
					$precio_base = (($_SESSION['mayoristas']) ? $mix['pd_precio_mayo'] : $mix['pd_precio']);
				} else {
					$peso_total += $mix['cantidad'] * (($_SESSION['mayoristas']) ? $mix['pd_peso_mayo'] : $mix['pd_peso']);
					$precio_total += $mix['cantidad'] * (($_SESSION['mayoristas']) ? $mix['pd_precio_mayo'] : $mix['pd_precio']);
				}
			...
			if (isset($precio_base)) {
				...
				$precio_base_reduc = ($peso_base * $precio_base) / (($_SESSION['mayoristas']) ? 1000 : 350);
			...
			if ($id_prod_creado == 0) {
				...
				if (mysqli_query($this->conn,$query)) {
					...
					$pr_codigo=$alias;
					$pr_precio = (($_SESSION['mayoristas']) ? ($precio_total*5) : $precio_total);
					$pr_variacion='-';
					$pr_valor='-';
					$pr_stock=100;
