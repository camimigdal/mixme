...
class mainClass {
	...
	public function Market()
	{
		...
		WHERE pd_destacado='si' AND status='publicado' ORDER BY pd_orden_dest ASC ";
	...
	public function ProductosHome()
	{
		...
		LEFT JOIN tbl_productos_parent ON tbl_productos.pd_id = tbl_productos_parent.pr_producto
		WHERE status='publicado' AND pd_exclusivo='si' ORDER BY pd_orden_exclusivo ASC";
	...
	public function filtrosCategorias() {
		...
		if(isset($_GET['cat'])){
			...
			$result=mysqli_query($this->conn,"SELECT * FROM `tbl_categorias` WHERE ct_padre=0 ORDER BY ct_orden ASC");
