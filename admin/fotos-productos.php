        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                function getEmployee() {
                    $.ajax({
                      type: "GET",  
                      url: "include/generar_fotos.php?id="+<?php echo $id ?>,
                      dataType: "json",       
                      success: function(response)  
                      {
                        var html_data = '';
                        for (var i = 0; i < response.length; i++) {
                             html_data += '<tr><td><img id="thumbnil" src="../img/productos/'+response[i].im_nombre+'" class="img-thumbnail" width="200"/></td>';

                             html_data += '<td><a href="#" data-name="im_orden" id="im_orden" data-type="number" data-pk="'+response[i].im_id+'" class="editable editable-click ';
                             if(response[i].im_orden=="") html_data += 'editable-empty">Empty</a></td>'; else html_data += '">'+response[i].im_orden+'</a></td>';

                             html_data += '<td><a href="upload_crop.php?nom_fot='+response[i].nombreFot+'" class="btn btn-success btn-lg"><i class="fa fa-edit"></i> Editar</a></td>';

                             html_data += '<td><a href="fotos-productos.php?id='+response[i].im_producto+'&action=delete&img='+response[i].im_nombre+'" data-confirm="Está seguro que desea eliminar?" class="btn btn-danger btn-lg"><i class="fa fa-trash"></i> Remover</a></td></tr>';
                         };
                ...
                make_editable_col('#example tbody','a#im_orden','include/generar_fotos.php?action=edit','Orden');
